...
Transformer 的关键变化是用注意力机制替代传统循环结构，使模型可以并行处理序列并学习长距离依赖。

现代大语言模型多采用 decoder-only 架构，通过不断预测下一个 token 来生成文本。理解这一点，有助于解释为什么提示词顺序、上下文长度和采样参数会影响输出。
