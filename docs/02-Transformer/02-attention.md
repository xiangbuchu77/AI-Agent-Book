...
Attention 可以理解为模型在生成当前位置表示时，对上下文中不同 token 分配不同权重。Self-Attention 让序列内部的 token 彼此建立关系。

工程上不需要手写注意力公式，但需要知道：模型并不是平均阅读上下文。重要信息的位置、表达方式和重复程度，都会影响它被使用的概率。
