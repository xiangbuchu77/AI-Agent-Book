...
当 Agent 任务变成长流程、多分支、可恢复和可观测时，简单 while 循环会越来越难维护。LangGraph 用图结构表达工作流，让控制流更加明确。

LangGraph 的价值在于把 Agent 行为拆成节点、边和状态，使开发者可以控制路径、保存进度、处理人工介入和调试执行过程。
